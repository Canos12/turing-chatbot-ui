...
export const Sidebar: FC<Props> = ({
		...
		if (searchTerm) {
			setFilteredConversations(
				conversations.filter((conversation) => {
					const searchable =
						conversation.name.toLocaleLowerCase() + " " + conversation.messages.map((message) => message.content).join(" ");
	...
	return (
		<aside
			className={`h-full transition-all flex flex-none space-y-2 p-2 flex-col bg-[#111111] w-[260px] z-50 sm:relative sm:top-0 fixed top-0 bottom-0`}
		>
					...
					>
						<Conversations
							...
							conversations={filteredConversations.filter(
								(conversation) => conversation.folderId === 0 || !folders[conversation.folderId - 1]
							)}
							selectedConversation={selectedConversation}
							onSelectConversation={onSelectConversation}
							onDeleteConversation={handleDeleteConversation}
